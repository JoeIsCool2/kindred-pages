...
function App() {
  ...
  const launchReadinessReportText = () => {
    const snapshot = getLaunchSnapshot(site);
    const checks = getLaunchChecks(site);
    const issues = getLaunchIssues(site, progress);
    const metadata = shareMetadata();
    const plan = getPlanDetails(site.plan);

    return [
      `Launch readiness report for ${site.name}`,
      '',
      `Readiness: ${progress}%`,
      `Launch status: ${site.launchStatus}`,
      `Recommended next step: ${issues[0]?.label || (site.launchStatus === 'Published' ? 'Preserve the archive and send aftercare follow-up' : 'Publish and invite guests')}`,
      '',
      'What is ready',
      ...snapshot.map((item) => `${item.status === 'Ready' ? '[ready]' : '[review]'} ${item.label}: ${item.done} of ${item.total} essentials complete - ${item.detail}`),
      '',
      'What still needs attention',
      ...(issues.length ? issues.map((issue) => `[${issue.tone}] ${issue.label}: ${issue.detail}`) : ['No launch issues found.']),
      '',
      'Family approvals',
      `Family approval: ${approvalComplete(site) ? 'Complete' : 'Needs review'} (${site.launchApproval.reviewer || 'reviewer not recorded'})`,
      `Privacy review: ${privacyReviewComplete(site) ? 'Complete' : 'Needs review'} (${site.privacyReview?.reviewer || 'reviewer not recorded'})`,
      `Sensitive details review: ${sensitiveReviewComplete(site) ? 'Complete' : 'Needs review'} (${site.sensitiveReview?.reviewer || 'reviewer not recorded'})`,
      `Accessibility review: ${site.accessibilityChecks.filter((check) => check.done).length} of ${site.accessibilityChecks.length} complete`,
      '',
      'Public page choices',
      `Memorial link: ${shareUrl}`,
      `Share destination: ${metadata.url}`,
      `Privacy: ${site.privacy}`,
      `Search visibility: ${site.searchVisibility}`,
      `Robots: ${metadata.robots}`,
      `Guest sharing: ${site.allowGuestSharing ? 'Allowed' : 'Family only'}`,
      `Public photos: ${publicPhotos(site).length}`,
      `Approved memories: ${site.memories.length}`,
      `Pending memories: ${site.pendingMemories.length}`,
      `Kept-private memories: ${site.rejectedMemories?.length || 0}`,
      '',
      ...
      `Gathering type: ${site.gatheringType}`,
      `Service: ${site.serviceDate} at ${site.serviceTime}, ${site.servicePlace}`,
      `Guest contacts: ${site.rsvp.filter((guest) => guest.email || guest.phone).length} of ${site.rsvp.length}`,
      `Guest needs waiting: ${site.rsvp.filter((guest) => !guest.followUpDone && (guest.needs || guest.note)).length}`,
      `Open support needs: ${(site.supportNeeds || []).filter((need) => need.status === 'Open').length}`,
      `Guest updates: ${site.guestUpdates?.length || 0}`,
      '',
      'Payment, publishing, and preservation',
      `Plan: ${plan.name} (${plan.price}, ${plan.billing})`,
      `Checkout: ${site.checkoutStatus}`,
      `Domain: ${site.domainStatus}`,
      `Publishing: ${site.launchStatus}`,
      `Invites: ${site.inviteStatus}`,
      `Archive: ${site.archiveStatus}`,
      `Retention: ${site.retentionPlan}`,
      `Closure: ${site.closureStatus}`,
      '',
      'Launch checklist',
      ...checks.map((check) => `${check.done ? '[done]' : '[ ]'} ${check.label}`),
      '',
      'Generated by Kindred Pages'
    ].join('\n');
  };

  const copyLaunchReadinessReport = () => {
    copyText(launchReadinessReportText(), 'Launch readiness report');
    addActivity('Launch readiness report copied', 'A family-facing readiness summary with public, private, review, and launch details was copied.');
  };

  const downloadLaunchReadinessReport = () => {
    downloadTextFile(`${site.slug || 'kindred'}-launch-readiness-report.txt`, launchReadinessReportText());
    setToast('Readiness report downloaded');
    addActivity('Launch readiness report exported', 'A family-facing readiness summary was downloaded before launch.');
  };
...
function Editor({ active, site, progress, shareUrl, productionUrl, shareMetadata, copyShareMetadata, checkoutPacketText, copyCheckoutPacket, copyFamilyApprovalPacket, copyLaunchReadinessReport, downloadLaunchReadinessReport, copyFamilyHandoffPacket, copyClosurePacket, integrationChecks, applyGatheringPreset, update, updateMessage, updateLivestreamPlan, updateList, removeList, addMemory, addMilestone, addScheduleItem, addCareContact, addDayOfTask, addGuestFaq, addCustomCare, addSupportGift, addSupportNeed, addAftercareReminder, addAnniversaryCare, updateAnniversaryStatus, addThankYouRecipient, addCoadmin, copyCoadminInvite, requestAdminAccess, addServiceOrderItem, addProgramPerson, addServiceSelection, addGuestUpdate, addObituaryPlacement, updateObituaryPlacementStatus, addClosureRequest, updatePartner, addPartnerDraft, markHandoffReady, toggleAftercare, toggleThankYouSent, toggleDayOfTask, toggleGuestFollowUp, toggleGuestInvite, updateClosureRequestStatus, handlePhotos, setCoverPhoto, setSite, qrDataUrl, approveMemory, rejectMemory, addRsvp, importGuestList, toggleTask, toggleAccessibilityCheck, toggleApprovalCheck, updateLaunchApproval, markLaunchApproved, togglePrivacyReviewCheck, updatePrivacyReview, markPrivacyReviewed, toggleSensitiveReviewCheck, updateSensitiveReview, markSensitiveReviewed, downloadCsv, downloadCalendar, downloadProgram, downloadMemoryBook, downloadQrCards, downloadGuestGuide, downloadDayOfPacket, copyAftercarePacket, aftercarePacketText, dayOfPacketText, downloadJson, startCheckout, connectDomain, publishPage, sendInvites, copyText, guestGuideText, importArchive, resetSample }) {
  ...
  return (
    <Panel title="Privacy & Launch" intro="The page can be shared gently, privately, or through a funeral-home workflow.">
      ...
      <div className="launch-report-panel">
        <div className="section-line">
          <h3>Launch Readiness Report</h3>
          <span>{launchIssues.length ? 'Review before sharing' : 'Ready to hand off'}</span>
        </div>
        <div className="launch-report-summary">
          <FileHeart size={22} />
          <div>
            <strong>Give the family one plain-language launch summary</strong>
            <p>The report collects readiness, privacy, sensitive-detail review, public visibility, guest-care gaps, payment status, archive status, and the next recommended action.</p>
          </div>
        </div>
        <div className="launch-report-grid">
          <div>
            <span>Next action</span>
            <strong>{launchIssues[0]?.label || (site.launchStatus === 'Published' ? 'Preserve and follow up' : 'Publish and invite')}</strong>
          </div>
          <div>
            <span>Family reviews</span>
            <strong>{[approvalComplete(site), privacyReviewComplete(site), sensitiveReviewComplete(site)].filter(Boolean).length} of 3 complete</strong>
          </div>
          <div>
            <span>Public visibility</span>
            <strong>{site.privacy} · {shareMetadata().robots}</strong>
          </div>
          <div>
            <span>Records included</span>
            <strong>{site.memories.length + site.pendingMemories.length} memories · {site.rsvp.length} guests</strong>
          </div>
        </div>
        <div className="action-row">
          <button className="primary" onClick={copyLaunchReadinessReport}><Download size={17} /> Copy report</button>
          <button className="secondary" onClick={downloadLaunchReadinessReport}><FileHeart size={17} /> Download report</button>
        </div>
      </div>
